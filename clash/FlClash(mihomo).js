...
const _CN_CARRIER_LABEL_RE =
  /中国(?:电信|联通|移动|铁通)|电信|联通|移动|铁通|\bchina[\s_-]*(?:telecom|unicom|mobile)\b|\b(?:chinatelecom|chinaunicom|chinamobile)\b/i;
function _stripCnCarrierLabels(name) {
  return name.replace(
    /中国(?:电信|联通|移动|铁通)|电信|联通|移动|铁通|\bchina[\s_-]*(?:telecom|unicom|mobile)\b|\b(?:chinatelecom|chinaunicom|chinamobile)\b/gi,
    " ",
  );
}
...
function sckiResolveSubscriptionAdapterProfile(requestedProfile) {
  ...
  var selected =
    SCKI_SUBSCRIPTION_ADAPTER_PROFILES[requested] ||
    SCKI_SUBSCRIPTION_ADAPTER_PROFILES[
      SCKI_SUBSCRIPTION_ADAPTER_PROFILE_DEFAULT
    ];
...
var SckiSubscriptionAdapter = (function () {
  ...
  function isIPv6(value) {
    ...
    if (compressedAt !== -1 && text.indexOf("::", compressedAt + 2) !== -1)
      return false;
    var groups = text.split(":").filter(function (group) {
      return group !== "";
    });
    if (
  ...
  function normaliseDomain(value) {
    ...
    if (!domain || domain.length > 253 || /[\x00-\x20\\/@:?#[\]]/.test(domain))
      return "";
  ...
  function normaliseResolver(value) {
    ...
    if (
      !resolver ||
      resolver.length > NODE_DNS_HINT_LIMITS.stringLength ||
      /[\x00-\x20]/.test(resolver)
    )
      return "";
    var lowered = resolver.toLowerCase();
    if (
      resolver.indexOf("#") !== -1 ||
      /^(?:system|dhcp)$/i.test(resolver) ||
      /^rcode:/i.test(resolver)
    )
      return "";
    ...
    var match = resolver.match(
      /^(udp|tcp|tls|https|quic):\/\/(\[[0-9a-fA-F:.]+\]|[A-Za-z0-9.-]+)(?::(\d{1,5}))?(\/[^\s]*)?$/i,
    );
    if (!match) return "";
  ...
  function normaliseResolverList(value, snapshot) {
    ...
    for (
      var i = 0;
      i < rawValues.length && i < NODE_DNS_HINT_LIMITS.values;
      i++
    ) {
    ...
    if (rawValues.length > NODE_DNS_HINT_LIMITS.values)
      recordReject(snapshot, rawValues.length - NODE_DNS_HINT_LIMITS.values);
    return output;
  }
  ...
  function normaliseHostValues(value, snapshot) {
    ...
    for (
      ...
      if (
        !host ||
        host.length > NODE_DNS_HINT_LIMITS.stringLength ||
        (!ipv6Literal && /[\x00-\x20\\/@?#[\]]/.test(host))
      ) {
        recordReject(snapshot);
        continue;
      }
      if (ipv6Literal) {
        ...
        pushUnique(ipValues, host, function (entry) {
        ...
        pushUnique(redirects, host, function (entry) {
          return String(entry).toLowerCase();
        });
      }
    }
  ...
  function normalisePattern(value) {
    ...
    if (
      !pattern ||
      pattern.length > 253 ||
      /[\x00-\x20\\/@:?#[\]]/.test(pattern) ||
      pattern === "*"
    )
      return "";
    if (pattern.slice(0, 2) === "+.")
      return normaliseDomain(pattern.slice(2)) ? pattern : "";
    if (pattern.charAt(0) === ".")
      return normaliseDomain(pattern.slice(1)) ? pattern : "";
    if (pattern.indexOf("*") !== -1) {
      ...
      for (var i = 0; i < labels.length; i++) {
        if (
          labels[i] !== "*" &&
          !/^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/.test(labels[i])
        )
          return "";
      }
      return pattern;
  ...
  function patternMatches(pattern, domain) {
    if (pattern.slice(0, 2) === "+.") {
      ...
      return (
        domain === plusBase ||
        domain.slice(-(plusBase.length + 1)) === "." + plusBase
      );
    }
    if (pattern.charAt(0) === ".") {
      ...
      return (
        domain !== dotBase &&
        domain.slice(-(dotBase.length + 1)) === "." + dotBase
      );
    }
    if (pattern.indexOf("*") !== -1) {
      ...
      for (var i = 0; i < patternLabels.length; i++) {
        if (patternLabels[i] !== "*" && patternLabels[i] !== domainLabels[i])
          return false;
      }
      return true;
    }
  ...
  function patternScore(pattern) {
    if (pattern.indexOf("*") !== -1)
      return (
        3000 +
        pattern.split(".").length * 10 +
        pattern.replace(/\*/g, "").length
      );
    if (pattern.slice(0, 2) === "+." || pattern.charAt(0) === ".")
  ...
  function buildSourceView(source, activeDomains, snapshot) {
    ...
    activeDomains.forEach(function (domain) {
      ...
      if (hasOwn(source, domain + ".") && !hasOwn(view, domain + "."))
        view[domain + "."] = source[domain + "."];
    });
  ...
  function resolveProfile(runtimeProfile) {
    var requested =
      typeof runtimeProfile === "string"
        ? runtimeProfile
        : isPlainObject(runtimeProfile) && typeof runtimeProfile.id === "string"
          ? runtimeProfile.id
          : "";
  ...
  function addPolicy(snapshot, domain, values) {
    ...
    if (
      snapshot.resolvers.length + newResolvers.length >
      NODE_DNS_HINT_LIMITS.resolvers
    ) {
      recordReject(snapshot, values.length);
      return false;
    }
    snapshot.policy[domain] = values.slice();
    newResolvers.forEach(function (resolver) {
      snapshot.resolvers.push(resolver);
    });
    return true;
  }
  ...
  function captureNodeDns(sourceConfig, activeNodeServers, runtimeProfile) {
    ...
    var servers = activeNodeServers.slice(
      0,
      NODE_DNS_HINT_LIMITS.activeNodeServers,
    );
    if (activeNodeServers.length > NODE_DNS_HINT_LIMITS.activeNodeServers)
      recordReject(
        snapshot,
        activeNodeServers.length - NODE_DNS_HINT_LIMITS.activeNodeServers,
      );
    ...
    var sourceDns = isPlainObject(sourceConfig.dns)
      ? sourceConfig.dns
      : Object.create(null);
    var sourceProxyResolvers = [];
    if (
      profile.nodeDnsProjection === "adaptive" &&
      hasOwn(sourceDns, "proxy-server-nameserver")
    ) {
      sourceProxyResolvers = normaliseResolverList(
        sourceDns["proxy-server-nameserver"],
        snapshot,
      );
    }
    var sourceNodePolicy = buildSourceView(
      sourceDns["proxy-server-nameserver-policy"],
      snapshot.domains,
      snapshot,
    );
    var sourceGlobalPolicy = buildSourceView(
    ...
    snapshot.domains.forEach(function (domain) {
      if (
        Object.keys(snapshot.policy).length >= NODE_DNS_HINT_LIMITS.policies
      ) {
        recordReject(snapshot);
        return;
      }
      var selection = selectForDomain(
        sourceNodePolicy,
        domain,
        normaliseResolverList,
        snapshot,
      );
      if (!selection.matched)
        selection = selectForDomain(
          sourceGlobalPolicy,
          domain,
          normaliseResolverList,
          snapshot,
    ...
    Object.keys(snapshot.policy).forEach(function (domain) {
      pushUnique(hostTargets, domain);
    });
    var sourceHosts = buildSourceView(
      sourceConfig.hosts,
      hostTargets,
      snapshot,
    );
    hostTargets.forEach(function (domain) {
      ...
      if (hasValues(selection.value))
        snapshot.hosts[domain] = copyValue(selection.value);
    });
  ...
  function isCanonicalResolverValues(values) {
    if (
      !Array.isArray(values) ||
      !values.length ||
      values.length > NODE_DNS_HINT_LIMITS.values
    )
      return false;
    ...
    for (var i = 0; i < values.length; i++) {
      if (
        ...
        normaliseResolver(values[i]) !== values[i]
      )
        return false;
  ...
  function isCanonicalHostValue(value) {
    ...
    return (
      ...
      hasValues(normalised) &&
      sameValues(normalised, value)
    );
  }
  ...
  function validateSnapshot(snapshot, profile) {
    if (
      ...
      !isPlainObject(snapshot.hosts)
    )
      return { ok: false, reason: "invalid-snapshot" };
    if (snapshot.profile !== profile.id)
      return { ok: false, reason: "profile-mismatch" };
    if (
      !Array.isArray(snapshot.domains) ||
      snapshot.domains.length > NODE_DNS_HINT_LIMITS.domains
    )
      return { ok: false, reason: "invalid-snapshot" };
    ...
    for (
      var domainIndex = 0;
      domainIndex < snapshot.domains.length;
      domainIndex++
    ) {
      var activeDomain = snapshot.domains[domainIndex];
      if (
        typeof activeDomain !== "string" ||
        normaliseDomain(activeDomain) !== activeDomain ||
    ...
    for (var policyDomain in snapshot.policy) {
      ...
      if (
        policyKeys.length >= NODE_DNS_HINT_LIMITS.policies ||
        !activeDomains[policyDomain] ||
        normaliseDomain(policyDomain) !== policyDomain ||
        !isCanonicalResolverValues(snapshot.policy[policyDomain])
      ) {
        return { ok: false, reason: "invalid-snapshot" };
      }
    ...
    for (var hostDomain in snapshot.hosts) {
      ...
      if (
        hostKeys.length >= NODE_DNS_HINT_LIMITS.hosts ||
        !allowedHostDomains[hostDomain] ||
        normaliseDomain(hostDomain) !== hostDomain ||
        !isCanonicalHostValue(snapshot.hosts[hostDomain])
      ) {
        return { ok: false, reason: "invalid-snapshot" };
      }
  ...
  function applyNodeDns(repositoryConfig, snapshot, runtimeProfile) {
    var profile = resolveProfile(runtimeProfile);
    if (profile.nodeDnsProjection === "off")
      return buildReport(profile, snapshot, false, "profile-off");
    if (
      !isPlainObject(repositoryConfig) ||
      !isPlainObject(repositoryConfig.dns)
    ) {
    ...
    if (!validation.ok)
      return buildReport(profile, snapshot, false, validation.reason);
    var policyKeys = validation.policyKeys;
    var hostKeys = validation.hostKeys;
    if (!policyKeys.length && !hostKeys.length)
      return buildReport(profile, snapshot, false, "no-hints");

    if (policyKeys.length) {
      repositoryConfig.dns["proxy-server-nameserver-policy"] =
        Object.create(null);
      policyKeys.forEach(function (domain) {
        repositoryConfig.dns["proxy-server-nameserver-policy"][domain] =
          snapshot.policy[domain].slice();
      });
    } else {
      delete repositoryConfig.dns["proxy-server-nameserver-policy"];
    }
    if (!isPlainObject(repositoryConfig.hosts))
      repositoryConfig.hosts = Object.create(null);
    hostKeys.forEach(function (domain) {
      if (!hasOwn(repositoryConfig.hosts, domain))
        repositoryConfig.hosts[domain] = copyValue(snapshot.hosts[domain]);
    });
...
function overwriteGeneral(config, nodeDnsHints) {
  ...
  Object.keys(dnsHosts).forEach(function (k) {
    config.hosts[k] = dnsHosts[k].slice();
  });
  var bootstrapDns = [
  ...
  return SckiSubscriptionAdapter.applyNodeDns(
    config,
    nodeDnsHints,
    SCKI_SUBSCRIPTION_ADAPTER_PROFILE,
  );
}
...
function cleanupSubscription(config) {
  ...
  if (config["rule-providers"]) {
    Object.keys(config["rule-providers"]).forEach(function (k) {
      delete config["rule-providers"][k];
    });
  }
  log(
    `[${VERSION}] Cleared ${removed} subscription proxy-groups; rebuilt from Smart-Config-Kit baseline`,
  );
}
// ================================================================
//  模块 K：注入智能 TLS 指纹
...
function sortProxyGroups(config) {
  ...
  var globalGroup = null,
    restSmartGroups = [];
  for (var s = 0; s < smartGroups.length; s++) {
    if (smartGroups[s].name === SMART.GLOBAL) {
      globalGroup = smartGroups[s];
    } else {
      restSmartGroups.push(smartGroups[s]);
    }
  }
  config["proxy-groups"].splice(0, config["proxy-groups"].length);
  if (globalGroup) {
  ...
  for (var i = 0; i < bizGroups.length; i++) {
    config["proxy-groups"].push(bizGroups[i]);
  }
  for (var j = 0; j < restSmartGroups.length; j++) {
    config["proxy-groups"].push(restSmartGroups[j]);
  }
  for (var k = 0; k < otherGroups.length; k++) {
    config["proxy-groups"].push(otherGroups[k]);
  }
}
...
function main(config) {
  try {
    ...
    var activeNodeServers = collectActiveSubscriptionNodeServers(
      config.proxies,
    );
    var nodeDnsHints = SckiSubscriptionAdapter.captureNodeDns(
      config,
      activeNodeServers,
      SCKI_SUBSCRIPTION_ADAPTER_PROFILE,
    );
