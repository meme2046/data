...
var ACC_BANK_RULES = [
  ...
  return "RULE-SET,acc-bank-" + cc.toLowerCase() + "," + BIZ.PAYMENTS;
});
var ACC_VF_RULES = ["wise", "monzo", "revolut"].map(function (svc) {
  return "RULE-SET,acc-vf-" + svc + "," + BIZ.PAYMENTS;
});
var ACC_FAKE_LOCATION_RULES = [
  "bilibili",
  "kuaishou",
  "xigua",
  "weibo",
  ...
  return "RULE-SET,acc-fl-" + app + "," + BIZ.CNMEDIA;
});
...
function injectRuleProviders(config) {
  ...
  const metaDomain = (id, name) => {
    config["rule-providers"][id] = {
      ...
      behavior: "domain",
      format: "mrs",
      url: `${META}/geosite/${name}.mrs`,
      path: `./ruleset/meta-${name}.mrs`,
  ...
  const metaIpCidr = (id, name) => {
    config["rule-providers"][id] = {
      ...
      behavior: "ipcidr",
      format: "mrs",
      url: `${META}/geoip/${name}.mrs`,
      path: `./ruleset/meta-ip-${name}.mrs`,
      interval: nextInterval(),
      proxy: RP_PROXY,
    };
  };
  const bm7 = (id, name) => {
    ...
    config["rule-providers"][id] = {
      type: "http",
      behavior: "classical",
      url: `${cdn}/${name}/${name}.yaml`,
      path: `./ruleset/bm7-${name}.yaml`,
      interval: nextInterval(),
      proxy: RP_PROXY,
    };
  };
  const bm7Custom = (id, dir, file) => {
    ...
    config["rule-providers"][id] = {
      type: "http",
      behavior: "classical",
      url: `${cdn}/${dir}/${file}.yaml`,
      path: `./ruleset/bm7-${id}.yaml`,
      interval: nextInterval(),
      proxy: RP_PROXY,
    };
  };
  ...
  config["rule-providers"]["vpsdance-ai-coding"] = {
    type: "http",
    behavior: "classical",
  ...
  console.log(
    `[${VERSION}] Injected ${count} rule-providers (base=${RP_BASE}s step=${RP_STEP}s spread=${_rpIdx * RP_STEP}s/${((_rpIdx * RP_STEP) / 60).toFixed(1)}min)`,
  );
}
...
function injectRules(config) {
  config.rules = [
    ...
    ...RUSTDESK_WORK_PROCESS_NAMES.map(
      (name) => `PROCESS-NAME,${name},${BIZ.WORK}`,
    ),
    ...LOCAL_TOOL_DIRECT_PROCESS_NAMES.map(
      (name) => `PROCESS-NAME,${name},DIRECT`,
    ),
    ...
    ...GEO_REGIONS_INTL.map(
      (r) =>
        `RULE-SET,acc-geo-d-${r.toLowerCase().replace(/_/g, "-")},${BIZ.INTL_SITE}`,
    ),
    ...GEO_REGIONS_INTL.map(
      (r) =>
        `RULE-SET,acc-geo-ip-${r.toLowerCase().replace(/_/g, "-")},${BIZ.INTL_SITE},no-resolve`,
    ),
